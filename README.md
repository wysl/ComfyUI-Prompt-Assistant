...
<details open>
<summary><strong>[2026-08-16] 🔥当前开发版</strong></summary>

**Changes:**
* **参考提示词整目录选择**：多媒体参考提示词库可通过文件夹复选框一次选择或取消该目录及全部子目录中的 TXT 文件，并显示目录内文件总数和部分选择状态。
* **一键清空已选文件**：选择窗口标题栏新增清空按钮，可一次移除全部已选提示词，再按“确定”保存修改。
...
* **兼容大模型标题装饰**：最终请求再次明确逐字 `SHOT 1:` 与 `Audio:` 标题；节点会自动规范化模型偶发输出的 Markdown 加粗、标题符号及 `Shot 1 (0s-2s):` 时间写法，避免有效 H3 Base 提示词被误判失败。
* **自动修复 Base 关键帧锚点**：大模型若把 I2VA/L2VA/FL2VA 的 `<Picture N>` 改写成自然语言代称，节点会依据 H3 Context 已确定的首末帧角色，将缺失标签补回正确 Shot；Ref2VA 与不存在的媒体编号仍严格报错。
...
打开节点上的“选择参考提示词”窗口后，根目录只显示目录；进入某个目录后，只显示该层的子目录和 TXT 文件。点击文件夹左侧复选框，可一次选择或取消该文件夹及全部子目录中的 TXT 文件；半选状态表示只选择了其中一部分。标题栏的清空按钮可移除全部已选文件。已选文件在切换目录后仍会保留，可在“已选”视图中上移、下移或移除。文件顺序同时也是冲突优先级：越靠后的文件优先级越高。
